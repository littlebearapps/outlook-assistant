...
| `searchExpression="from:…"` / `subject:"…"` returns 0 on a personal account | Fixed in v3.10.0 — field-scoped `$search` expressions return nothing on personal Outlook.com accounts even when matching mail exists. They are now translated into the equivalent OData filters and retried (strategy `raw-kql-translated`). Unscoped expressions were never affected. (#217) |
